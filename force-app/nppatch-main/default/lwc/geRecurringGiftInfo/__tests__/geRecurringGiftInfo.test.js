...
jest.mock(
    "@salesforce/label/c.geOpenEndedGiftSchedule",
    () => {
...
jest.mock(
    "@salesforce/label/c.geFixedGiftSchedule",
    () => {
        return {
            default:
                "Recurring Schedule Information: {0} {1} donation starting on {2} and ending after {3} installments.",
        };
    },
    { virtual: true }
);
...
describe("c-ge-modal-recurring-donation", () => {
    ...
    describe("render behavior", () => {
        ...
        it("renders expected text for open ended schedules", async () => {
            const infoCard = setup(100, "2022-01-31", {
                [scheduleFields.recurringType]: "Open",
                [scheduleFields.effectiveDate]: "2022-01-31",
                [scheduleFields.installmentPeriod]: "Monthly",
            });
            ...
            expect(schedule.textContent).toBe(
                "Recurring Schedule Information: $100.00 monthly donation starting on January 31, 2022."
            );
        });
        ...
        it("renders 0 for NaN donation amount", async () => {
            const infoCard = setup(undefined, "2022-01-31", {
                [scheduleFields.recurringType]: "Open",
                [scheduleFields.effectiveDate]: "2022-01-31",
                [scheduleFields.installmentPeriod]: "Monthly",
            });
            ...
            expect(schedule).toBeDefined();
            expect(schedule.textContent).toBe(
                "Recurring Schedule Information: $0.00 monthly donation starting on January 31, 2022."
            );
        });
