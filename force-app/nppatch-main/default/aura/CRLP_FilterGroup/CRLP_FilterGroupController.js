({
    ...
    doInit: function (cmp, event, helper) {
        ...
        action.setCallback(this, function (response) {
            ...
            if (state === "SUCCESS") {
                ...
                var actions = [
                    { label: labels.edit, name: "edit" },
                    { label: labels.delete, name: "delete" },
                ];

                var filterRuleColumns = [
                    { label: labels.object, fieldName: "objectLabel", type: "string" },
    ...
    onCancel: function (cmp, event, helper) {
        if (
            (cmp.get("v.mode") === "clone" || cmp.get("v.mode") === "create") &&
            cmp.get("v.activeFilterGroupId") === null
        ) {
    ...
    onQueueFilterRuleSave: function (cmp, event, helper) {
        ...
        if (mode !== "delete") {
            ...
            if (canSave) {
                ...
                filterRule.operationLabel = helper.retrieveFieldLabel(
                    filterRule.operationName,
                    cmp.get("v.filteredOperators")
                );
                ...
                if (filterRule.operationName === "In_List" || filterRule.operationName === "Not_In_List") {
                    ...
                    filterRule.fieldName === "RecordTypeId" &&
                    (filterRule.operationName === "Equals" || filterRule.operationName === "Not_Equals")
                ) {
                    filterRule.valueLabel = helper.retrieveFieldLabel(
                        filterRule.value,
                        cmp.get("v.filterRuleConstantPicklist")
                    );
                } else {
