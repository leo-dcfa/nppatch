({
                ...
                if (!(type === undefined || type === null)) {
                    allFields.forEach(function (field) {
                        if (field !== undefined) {
                            ...
                            if (type === "REFERENCE") {
                                //rolling up into a lookup field
                                if (field.type === "REFERENCE" || field.type === "ID") {
                                    // possible detail fields are IDs and other lookup fields because they store IDs
                                    if (summaryFieldReferenceTo && field.referenceTo === summaryFieldReferenceTo) {
                                        // make sure the referenced objects match
                                ...
                                ((type === "PICKLIST" || type === "MULTIPICKLIST") && field.type === "STRING") ||
    ...
    onChangeOperation: function (cmp, operation) {
        ...
        if (amountObject) {
            ...
            var amountFieldLabel = this.retrieveFieldLabel(
                cmp.get("v.activeRollup.amountField"),
                cmp.get("v.amountFields")
            );
            if (!amountFieldLabel) {
                cmp.set("v.activeRollup.amountField", null);
    ...
    onChangeSummaryField: function (cmp, value, label) {
        ...
        this.updateAllowedOperations(cmp);
        var operationLabel = this.retrieveFieldLabel(
            cmp.get("v.activeRollup.operation"),
            cmp.get("v.allowedOperations")
        );
        if (!operationLabel) {
            cmp.set("v.activeRollup.operation", "");
            this.onChangeOperation(cmp, "");
        ...
        if (amountObject) {
            ...
            var detailFieldLabel = this.retrieveFieldLabel(
                cmp.get("v.activeRollup.detailField"),
                cmp.get("v.detailFields")
            );
            if (!detailFieldLabel) {
                cmp.set("v.activeRollup.detailField", null);
    ...
    renderDetailField: function (cmp, operation, rollupLabel, renderMap) {
        if (
            (operation === "First" || operation === "Last" || operation === "Smallest" || operation === "Largest") &&
            rollupLabel
        ) {
            //enable detail field
    ...
    setDefaultAmount: function (cmp, labels, amountObjectName) {
        var summaryFieldType = this.retrieveFieldType(
            cmp,
            cmp.get("v.activeRollup.summaryField"),
            cmp.get("v.summaryFields")
        );
    ...
    setRollupType: function (cmp) {
        ...
        if (detailObject === labels.objectOpportunity && summaryObject === labels.objectAccount) {
            if (amountObject === labels.objectPartialSoftCredit) {
                ...
                rollupType.summaryObject = labels.objectAccount;
                rollupType.label =
                    labels.labelOpportunity + " -> " + labels.labelAccount + " (" + labels.softCredit + ")";
                cmp.set("v.activeRollup.rollupTypeObject", labels.objectAccountSoftCredit);
            } else {
                rollupType.name = labels.objectOpportunity;
                rollupType.summaryObject = labels.objectAccount;
                rollupType.label =
                    labels.labelOpportunity + " -> " + labels.labelAccount + " (" + labels.hardCredit + ")";
            ...
            if (amountObject === labels.objectPartialSoftCredit) {
                ...
                rollupType.label =
                    labels.labelOpportunity + " -> " + labels.labelContact + " (" + labels.softCredit + ")";
                cmp.set("v.activeRollup.rollupTypeObject", labels.objectPartialSoftCredit);
            } else {
                rollupType.name = labels.objectOpportunity;
                rollupType.summaryObject = labels.objectContact;
                rollupType.label =
                    labels.labelOpportunity + " -> " + labels.labelContact + " (" + labels.hardCredit + ")";
                cmp.set("v.activeRollup.rollupTypeObject", labels.objectOpportunity);
            }
            ...
            $A.getCallback(function () {
                ...
                action.setCallback(this, function (response) {
                    ...
                    if (state === "SUCCESS") {
                        ...
                        if (
                            deployResult &&
                            deployResult.completed === true &&
                            (deployResult.rollupItem || mode === "delete")
                        ) {
                            ...
                            if (mode === "delete") {
                                ...
                                helper.showToast(
                                    cmp,
                                    "success",
                                    cmp.get("v.labels.rollupSaveProgress"),
                                    cmp.get("v.labels.rollupSaveSuccess")
                                );
                            ...
                            if (counter < maxPollingCount) {
                                ...
                                helper.showToast(
                                    ...
                                    "info",
                                    cmp.get("v.labels.rollupSaveProgress"),
                                    cmp.get("v.labels.rollupSaveTimeout")
                                );
                                helper.toggleSpinner(cmp, false);
                                cmp.set("v.mode", "edit");
