...
expect.extend({
    toContainOptions(actual, expected) {
        const valueExists = (valueToCheck) => [...actual].some((actualValue) => actualValue.value === valueToCheck);
        const lengthCheck = actual.length === expected.length;
        const pass = lengthCheck && [...expected].every((expectedValue) => valueExists(expectedValue));
